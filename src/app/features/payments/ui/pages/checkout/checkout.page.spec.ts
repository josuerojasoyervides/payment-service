...
import { PaymentProviderId, PaymentMethodType, CurrencyCode, PaymentIntent, PaymentError, FallbackAvailableEvent } from '../../../domain/models';
...
describe('CheckoutComponent', () => {
    ...
    describe('Selección de provider y método', () => {
        it('debe seleccionar provider correctamente', () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('Provider selected', 'CheckoutPage', { provider: 'paypal' });
        });
        ...
        it('debe seleccionar método correctamente', () => {
            ...
            expect(mockPaymentState.clearError).toHaveBeenCalled();
            expect(mockLogger.info).toHaveBeenCalledWith('Method selected', 'CheckoutPage', { method: 'spei' });
        });
    });
    ...
    describe('Proceso de pago', () => {
        ...
        it('debe procesar pago correctamente con provider y método válidos', () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('Payment request built', 'CheckoutPage', expect.any(Object));
        });
        ...
        it('debe auto-generar token en modo desarrollo para card', () => {
            ...
            component.processPayment();
            expect(mockBuilder.withOptions).toHaveBeenCalledWith(expect.objectContaining({ token: 'tok_visa_dev' }));
        ...
        it('debe manejar errores al construir request', () => {
            ...
            component.processPayment();
            expect(mockLogger.error).toHaveBeenCalledWith('Failed to build payment request', 'CheckoutPage', expect.any(Error));
        });

        it('debe iniciar correlación de logging', () => {
            component.processPayment();
            expect(mockLogger.startCorrelation).toHaveBeenCalledWith('payment-flow', expect.objectContaining({
    ...
    describe('Fallback', () => {
        it('debe confirmar fallback', () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith('Fallback confirmed', 'CheckoutPage', { provider: 'paypal' });
        });
